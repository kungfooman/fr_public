...
bool ImGui_ImplSdlGL3_CreateDeviceObjects()
{
...
#if defined(__EMSCRIPTEN__) || defined(ISOPENGLES2)
...
#if defined(EMSCRIPTEN) || defined(ISOPENGLES2)
		vertex_shader =
		"precision highp float;\n"
		"uniform mat4 ProjMtx;\n"
    ...
    GLint linkStatus;
    glGetProgramiv(g_ShaderHandle, GL_LINK_STATUS, &linkStatus);


 if (linkStatus == 0)
    {
        GLint infoLogLength;
        glGetProgramiv(g_ShaderHandle, GL_INFO_LOG_LENGTH, &infoLogLength);
		if (infoLogLength == 0)
			infoLogLength = 1024;
        ...
        if (infoLogLength > 1)
        {
            ...
            glGetProgramInfoLog(g_ShaderHandle, static_cast<GLsizei>(infoLog.size()), nullptr, &infoLog[0]);
